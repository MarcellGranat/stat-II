      ...
      tabPanel(
        ...
        sidebarLayout(
          sidebarPanel(
            ...
            numericInput("p", "Mintabeli arány (%)", min = 0, max = 100, value = 50),
...
# ui: Két várható érték különbségére -------------------------------------------------
      tabPanel(
...
# ui: Két szórás összehasonlítása -----------------------------------------
      tabPanel(
...
# S: Két várható érték különbségére -----------------------------------------------
...
# S: Két szórás összehasonlítása --------------------------------------------------------
  ...
  output$var_test <- renderText({
    ...
    x_4_2 <- rnorm(n_x, mean = 0, sd = sigma_x)
    y_4_2 <- rnorm(n_y, mean = 0, sd = sigma_y)

    if (var(x_4_2) > 0) {
      x_4_2 <- x_4_2 * (sigma_x / var(x_4_2)^0.5)
    }
    if (var(y_4_2) > 0) {
      y_4_2 <- y_4_2 * (sigma_y / var(y_4_2)^0.5)
    }
    test_4_2 <- var.test(x_4_2, y_4_2, alternative = test_side)
    dontes_4_2 <- "nincs"

    if (!is.na(test_4_2$p.value)) {
      if (test_4_2$p.value > alpha / 100) {
        print("0")
        if (test_side == "two.sided") {
          print("1")
